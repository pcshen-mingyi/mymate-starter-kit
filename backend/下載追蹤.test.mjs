 ...
 *
 * Apps Script 不能在本機執行，所以把「寫哪一列、當日新增算多少」的判斷
...
console.log("── 第一次抓取：當日新增必須是空值而非 0 ──");
{
  ...
  check("當日新增為 null（→ 寫空白）", plan.every((p) => p.delta === null));
...
console.log("\n── 重跑時當日新增要跟前一天比，不能跟今天早先那次比 ──");
{
  ...
  check("當日新增 = 9 − 5 = 4（跟昨天比，不是 9 − 8 = 1）", plan[0].delta === 4,
    "實際: " + plan[0].delta);
}
...
console.log("\n── 第二天抓取：當日新增等於差值 ──");
{
  ...
  check("v2.0 當日新增 = 12 − 5 = 7", plan[0].delta === 7);
...
console.log("\n── 多天累積：只跟最近一天比 ──");
{
  ...
  check("當日新增 = 11 − 10 = 1", plan[0].delta === 1);
}
